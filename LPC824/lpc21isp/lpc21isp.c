...
static void SearchSerialPort(ISP_ENVIRONMENT *IspEnvironment)
{
	int j;
	DWORD rx_s, tx_s;
    DCB    dcb;
    COMMTIMEOUTS commtimeouts;
	static char portname[_MAX_PATH]; // Needs to be static!
	char buff[0x100];
	...
	for(j=255; j>0; j--)
	{
		...
		IspEnvironment->hCom = CreateFile(portname, GENERIC_READ | GENERIC_WRITE,0,NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,NULL);
		...
		if (IspEnvironment->hCom != INVALID_HANDLE_VALUE)
		{
			...
			SetCommState(IspEnvironment->hCom, &dcb);
			...
			do {
				ReadFile(IspEnvironment->hCom, &buff, sizeof(buff), &rx_s, NULL);
			} while(rx_s!=0);

			WriteFile(IspEnvironment->hCom, "?", 1, &tx_s, NULL);
			Sleep(100);

			buff[0]=0;
			ReadFile(IspEnvironment->hCom, &buff, strlen("Synchronized"), &rx_s, NULL);
			buff[rx_s]=0;

			if(strstr(buff, "Synchronized")!=NULL)
			{
				CloseHandle(IspEnvironment->hCom);
				printf("\nLPC microcontroller found at COM%d\n", j);
				fflush(stdout);
				IspEnvironment->serial_port=portname;
				// Create COMPORT.inc file
...
int List_FTDI_Devices (void)
{
	...
	if (numDevs > 1)
	{
		printf("More than one device detected.  Do not know which one to use!\n");
		for(j=0; j<(int)numDevs; j++)
		{
...
static void OpenSerialPort(ISP_ENVIRONMENT *IspEnvironment)
{
	...
	if((stricmp(IspEnvironment->serial_port, "-ft230") == 0) ||
		(stricmp(IspEnvironment->serial_port, "ft230") == 0) )
	{
	    if(FT_Open(List_FTDI_Devices(), &handle) != FT_OK)
	    {
	        printf("Can not open FTDI adapter.\n");
	    ...
	    { 
	    	if (lComPortNumber != -1)
	    	{
				sprintf(portname, "\\\\.\\COM%d", lComPortNumber);
				IspEnvironment->serial_port=portname;
	...
	else if((stricmp(IspEnvironment->serial_port, "-cbus") == 0) ||
		(stricmp(IspEnvironment->serial_port, "cbus") == 0) )
	{
		FTDI_Set_CBUS_Mode(0); // Restore default operation of CBUS0 and CBUS3
		GetOut(0);
	}

	else if ((stricmp(IspEnvironment->serial_port, "-FindPort") == 0) ||
		    (stricmp(IspEnvironment->serial_port, "FindPort") == 0) )
    {
		// Search for serial port with LPC attached if requested - Jesus' hack...
		SearchSerialPort(IspEnvironment);
    }
...
static void ReadArguments(ISP_ENVIRONMENT *IspEnvironment, unsigned int argc, char *argv[])
{
    ...
    if (argc < 5)
    {
        ...
        DebugPrintf(1, "Syntax:  lpc21isp [Options] file[ file[ ...]] comport baudrate Oscillator_in_kHz\n\n"
                       ...
                       "         -FindPort    serch all available serial ports for LPC IC\n"
